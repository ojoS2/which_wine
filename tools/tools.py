...
def import_data(filename='./data/winemag_data_first150k.csv', separator=','):
    """
...
def column_description(column='all'):
    ...
    temp['description'] = """A few sentences from a sommelier describing the wine's taste, smell, look, feel, etc."""
    temp['designation'] = """The vineyard within the winery where the grapes that made the wine are from."""
    temp['points'] = """The number of points WineEnthusiast rated the wine on a scale of 1-100 (though they say they only post reviews for wines that score >=80)"""
    ...
    temp['region_2'] = """Sometimes there are more specific regions specified within a wine growing area (ie Rutherford inside the Napa Valley), but this value can sometimes be blank"""
    temp['variety'] = """The type of grapes used to make the wine (ie Pinot Noir)"""
    ...
    if column == 'all':
        ...
        for index in range(len(temp)):
            if temp.iloc[index, 0] > 0:
                print("""The feature {0} presents {1} missing values corresponding to {2}% of the entries""".format(temp.index[index], temp.iloc[index, 0],round(temp.iloc[index, 0]/ df.shape[0] * 100, 4)))  # nopep8: E128
            else:
        ...
        if temp[0] > 0:
            print("""The feature {0} presents {1} missing values corresponding to {2}% of the entries""".format(column, temp[0], round(temp[0] / df.shape[0] * 100, 4)))  # nopep8: E127
        else:
...
def drop_missing_column(column, df=pd.DataFrame({'A': []})):
    ...
    if temp[0] == 0:
        ...
        print("""{0} rows, or {1}% of the data frame rows were deleted.""".format(old - df.shape[0], round((1.0 - df.shape[0] / old), 5) * 100))   # nopep8: E127
        return df
...
def model_prob_tab(df, vectorizer_option=0, ngrams_limit=(1, 2),
    ...
    if vectorizer_option == 0:
        ...
        raise ValueError('''Option not reconized by the parameter vectorizer_option''')  # nopep8: E127
    X_train, X_test, y_train, y_test = train_test_split(df['description'],
...
def predict_and_print_probabilities(sample_size=100, min_descriptions=100):
    ...
    for index, row in prob_tab.iterrows():
        ...
        print('It is a ' + sample['variety'][index] + ''', according to the dataset. This variety presents {0} descriptions.'''.format(num_desc))  # nopep8: E127
...
def predict_and_print_recomendations(min_descriptions=100):
    ...
    df = import_data()
    df = select_number_of_descriptions(df, n=min_descriptions)
    print("""In the following, write the description of the desired wine. The description should have a description of expected sensations from the wine, followed by what you want to drink it with and end with how long you want it to be aged, or what age you want to drink it. Type examples to see examples from the database.""")   # nopep8: E127
    flag = input('''Type example (whitout spaces) to see the examples or type go to print the descriptions:\t''')
    if flag == 'example':
        while flag == 'example':
            temp = df.sample(n=1)
            print(f'The description: {temp["description"]} is associated to the variety {temp["variety"]}')
            print('\n\n')
            flag = input('''Type example (whitout spaces) to generate more examples or go to type the description. Any other words will exit the function:\t''')   # nopep8: E127
    if flag == 'go':
        print('''Type the phrases and press enter to go to the next phrase. Type exit to stop inputing and run the model''')   # nopep8: E127
        sample = []
        temp = input('Input the first description:\t')
        while temp != 'exit':
            sample.append(temp)
            temp = input("""Print another description or exit to run the model with the already given descriptions.""")
    ...
    if len(sample) == 0:
        ...
        for index, row in prob_tab.iterrows():
            ...
            print('\n\n')
            print(f'The description: \"{sample[index]}\" may be refeering to a:')
            for i in range(-1, -5, -1):
                print('''{0} with likelyhood of {1}%;'''.format(prob_list[i][0], round(prob_list[i][1], 6) * 100))   # nopep8: E127
# _________________________________________________
...
#os.rename(filename, 'winemag-data_first150k.csv')
